...
A stylised, DUI-based spawn selector for [QBX Core](https://github.com/Qbox-project/qbx_core) — a drop-in replacement for `qbx_spawn`. It hooks the same entry points qbx_core and qbx_multicharacter already call, so no other resource needs to change to use it. Players get an in-world camera scene with a prop-rendered DUI menu to pick their spawn location, instead of the default NUI list.
...
::: info
This resource replaces `qbx_spawn`. Don't run both at once.
:::
